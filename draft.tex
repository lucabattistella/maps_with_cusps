...
As regards surjectivity, since $\Phi$ is proper, and by upper semicontinuity of the dimension of the fiber \cite[\href{http://stacks.math.columbia.edu/tag/0D4I}{Tag 0D4I}]{stacks-project}, the locus in $\mathcal C_S'$ where the fiber of $\Phi$ is empty is open. The complement of its image in $S$ is the locus we were after.
 
\item[Step 3] Let $\mathcal{X}'$ be the image of $\mathcal Y$ under $\Pi$. This is a constructible substack of $\operatorname{Mor}_{\mathfrak{M}\times\mathfrak{M}'}(\mathcal C,\mathcal C')$ by Chevalley's theorem \cite[\href{http://stacks.math.columbia.edu/tag/054K}{Tag 054K}]{stacks-project}. Recall that, to show that a constructioble set is open (respectively closed), it is enough to check that it contains all the generisations of its points (respectively all the specialisations) \cite[\href{http://stacks.math.columbia.edu/tag/0903}{Tag 0903}]{stacks-project}. Finally, under Noetherian assumptions, two points related by specialisation/generisation are contained in the image of a DVR scheme \cite[\href{http://stacks.math.columbia.edu/tag/054F}{Tag 054F}]{stacks-project}.
...
\begin{enumerate}
\item\emph{$\phi$ contracts only weight zero components.} We show that this is open. Assume that $S$ is a DVR scheme with closed point $0$ and generic point $\eta$, and we are given $S\to\mathcal X'$ such that $\phi_0\colon\mathcal C_0\to \mathcal C_0'$ does not contract any positive weight component. Suppose, by contradiction, that there exists an irreducible component $W\subseteq C_{\eta}$ of positive weight $d_W$ which is contracted by $\phi_{\eta}.$ The contracted locus, i.e. $\left\{c\in \mathcal{C}_{S} | \dim_c\phi^{-1}(\phi(c))=1\right\}$, is closed by semicontinuity of fiber dimension and contains all the components $D_i\subseteq C_0$ to which $D$ specialises; at least one of them must have positive weight, since the sum of their weights is $d_W$, which is a contradiction.
...
First of all, we claim that the projection is \emph{representable by algebraic spaces}: by \cite[\href{http://stacks.math.columbia.edu/tag/04Y5}{Tag 04Y5}]{stacks-project} we only need to check that it is faithful, and by \cite[Theorem 2.2.5]{CONR} it is enough to look at geometric points; hence we are just saying that
$\operatorname{Aut}(\phi)\subseteq\operatorname{Aut}(C).$ 
 This follows from the fact that, if $\phi\in\mathcal X$, then it is dominant and of degree one.
Secondly, $\pr_{1|\mathcal X}$ is \emph{proper}: this can be seen using the valuative criterion
\bcd
\eta=\Spec(K)\ar[r]\ar[d] & \mathcal X \ar[d] \\
S=\Spec(R)\ar[r] & \mathfrak M
\ecd
Let $\pi \colon \mathcal C_S\to S$ be the family of nodal curves on $S$; there are three cases to consider:
\begin{enumerate}
\item the central fiber contains no elliptic tail, then so does $\mathcal C_\eta$, hence $\phi_\eta$ 
\marginpar{\begin{color}{red} It's right but confusing! Write it better!!!\end{color}}is an isomorphism and admits a unique extension to an isomorphism $\phi\colon \mathcal C_S\cong \mathcal C_S'.$

\end{enumerate}
If instead $\mathcal C_0$ has got an elliptic tail, then we have two possibilities:
\begin{enumerate}
\setcounter{enumi}{1}
\item $\mathcal C_{\eta}$ still has an elliptic tail, i.e. the node separating the weight zero, genus one curve from the rest of $\mathcal C_S$ is not smoothened. Then $\mathcal C_S$ is the push-out of a family of rational curves $\mathcal R_S$ and a family of genus one curves $\mathcal Z_S$.
...
\ecd
\marginpar{\begin{color}{red} Before writing the push down diagram we might need to change base. Write down the double push-out diagram\end{color}}
Locally around the node, $\phi_{\eta}$ is the contraction of the elliptic curve to the cusp, and, on the adjacent rational component, it is the normalisation of the cusp; by the universal property of push-outs, it can be uniquely extended to the same map on the central fiber (far from this node, $\phi_\eta$ is an isomorphism).
\item If $\mathcal C_S$ smoothens the elliptic tail, then the map is given on the generic fiber by a $\pi$-ample line bundle $\mathcal L_{\eta}$. Then the extension to the central fiber is determined by the unique extension $\mathcal L_0$ of $\mathcal L_{\eta}$ which is trivial on the minimal subcurve $Z\subseteq \mathcal C_0$ of arithmetic genus 1 and, possibly after twisting, is ample elsewhere. 
\end{enumerate}
Finally, observe that the map is bijective by construction and $\mathfrak M$ is normal, hence $\pi_{|\mathcal X}\colon\mathcal X\to \mathfrak M$ is an isomorphism by an application of Zariski's main theorem (as in \cite[\href{http://stacks.math.columbia.edu/tag/082I}{Tag 082I}]{stacks-project}).
 \end{proof}
