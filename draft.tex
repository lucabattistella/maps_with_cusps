...
\begin{prop}\label{1-stabilization-div}
 ...
 \ecd
\end{prop}
We prove a proposition through a series of Lemma s. The first one is a technical statement telling us that we can reduce to verify semi-ampleness and local freeness on families over DVR.
\begin{lem}{Reduction to DVR}\label{DVR}
Let $T$ the spectrum of a DVR with closed point $0$ and generic point $\eta$ and let $f\colon T\to\MM_1^{\rm{div}}$ such that $0\in \mathfrak{D}^1$ and $\eta\in \MM_1^{\rm{div}}\setminus \mathfrak{D}^1.$ Then 
...
\end{lem}
\begin{proof}
The argument is taken from \cite[Lemmma~3.7.2.2]{RSPW}.
...
Consider the caresian diagram 
\bcd
C'\ar[r,"f_C"]\ar[d,"\pi'"] & C\ar[d,"\pi"] \\
S'\ar[r,"f"] & S.
\ecd
To show that the above construction is compatible with base change it is enough to show that for any base change holds what we proved in Lemma~\ref{DVR}:
\begin{equation}\label{eq:generalbasechange}
f^*\pi_*\mathcal M\cong \pi'_*f^*\mathcal M.
\end{equation}
Since $\pi$ is flat we do have an isomorphism if we consider everything derived:
\begin{equation}\label{eq:derivedbasechange}
\rm{L}f^*\R\pi_*\mathcal M\cong \R\pi'_*\rm{L}f^*\mathcal M.
\end{equation}
First notice that the $H^0$ on the LHS of \eqref{eq:derivedbasechange} is exactly the LHS of \ref{eq:generalbasechange}.
