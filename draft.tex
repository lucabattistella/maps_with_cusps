...
\[ \dim_k R/I_p \]
where $I_p=\operatorname{Ann}_R(\widetilde R/R)$ is the conductor ideal at the singular point. \marginpar{prove it or at least say what numbers it gives}
\end{remark}
...
Thus it is enough to verify that $H^1(\hat{C},\hat{L})=0$ \cite{Wang}. Once done this we are going to obtain the smoothing of the original map by precomposing with the contraction $F\colon\cC\to\hat{\cC}\xrightarrow{\hat F}\PP^r$.
