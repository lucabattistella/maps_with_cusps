...
% For $m\geq\operatorname{min}(r,d)+n$, $\overline{\mathcal{M}}^m_{1,n}(\PP^r,\beta)$ is irreducible.
 ...
 \begin{dfn}
 Let $C$ a reduced curve and $p\in C$ a singular point, we define the genus of the singularity in $p$ to be the quantity: $g(p)=\delta(p)-b(p)+1.$  We are using the following notation: $\delta(p)=\dim_k\OO_{\overline C}/\OO_C,$ $\overline{C}$ being the normalization of $C$; $b(p)$ is the number of branches at $p.$
 \end{dfn}
...
\end{proof}
\begin{lem}
All of the Cohen-Macaulay genus ~$1$ singularities are smoothable.
\end{lem}
\begin{proof}
We prove this statement by exhibiting an explicit smoothing for this singularities.
Start considering two families $\cC^0\to T$ and $\cC^1\to T$ respectively of smooth rational curves and smooth elliptic curves, with $T$ a DVR scheme, e.g. consider two trivial families.
...
Viceversa, let us suppose that $f\colon C\to \PP^r$ factors through $\hat{f}\colon \hat{C}\to\PP^r$  with $\hat{C} $a genus $1$ singularity. Let $\hat{\cC}$ be a smoothing of $\hat{C}$ and $\hat{\mathcal L}$ an extension of $\hat{L}:=\hat{f}^*\OO_{\PP^r}(1)$ on the smoothing, which exists because deforming line bundles on curves is unobstructed. Then all we have to show is that the $r+1$ sections $\hat{s}_0,\dots, \hat{s}_{r+1}$ representing the map $\hat{f}$ extend to sections of $\hat{\mathcal{L}}.$
Thus it is enough to verify that $H^1(\hat{C},\hat{L})=0.$ 

Since all the curve we are considering are Cohen-Maculay, they have a dualising pure sheaf $\omega_{\hat{C}}.$
...
In this section: we show in two different ways that there exists a morphism at the level of weighted prestable curves $\mathfrak M_{1,n}^{\operatorname{wt}=d,\text{st}}\to\mathfrak M_{1,n}^{\operatorname{wt}=d,\text{st}}(1);$ introduce the spaces 
