...
and hence the map is smoothable. Viscardi claims that the map factors through the family $\cC'_T$, obtained by contracting $E$ to a tacnode. Notice that the image of $f'$ would still be $D$, since there is at most one indeterminacy point of $\cC'_T\dashrightarrow\PP^r_T$, located at the singularity. However in our example $f$ cannot factor through the tacnode. Indeed in that case we would have a birational morphism between two singular curves with the same $\delta$ invariant and the same normalisation, so $f'\colon C'\to D$ would be an isomorphism, which is a contradiction.
...
\[I_p=\operatorname{Ann}_R(\widetilde R/R)=\left\{f\in\, R\;\rvert\; f\cdot\widetilde{R}\subseteq R\right\}\]
 ...
 Using the explicit description of $\mathfrak{m}_R$ we gave in the classification Lemma we can easily understand the generators of $I_p$ and check that for a $(k,m-k)$ singularity  we have 
 \[ \dim_k R/I_p=m-k.\]
\end{remark}
...
We first show that:
\begin{claim}
 We can produce smoothings $\cC$ of $C$ and $\hat\cC$ of $\hat C$ with a contraction map $\cC\to\hat\cC$ compatible with the given $C\to\hat C$~\ref{rem:compatible_smoothing}.
 \end{claim}
\begin{proof}
 Assume that $\hat C$ has a $(k,m-k)$-singularity. 
Pick a smoothing $\cC$ of $C$ such that after resolving the $A_n$ singularities at the nodes, i.e. when looking at the semistable model with regular total space $\cC^{\rm{ss}}$, the following holds: call $E$ the maximal balanced genus one subcurve \cite[Proposition~2.11]{SMY1} contracted by $f^{\rm{ss}}$,
...
whose 
  resulting contraction $\bar{C}$ gives a $(m-k+h)$-fold point with $h\leq k$. Notice that $\bar{C}$ comes with a map $\rho$ to $\hat{C}$ and at this point use $\rho^*\hat{f}^*\OO_{\PP^r}(1)$ to perform the second contraction.  If necessary, we add markings $p_1+\dots p_N=\Sigma$ on the degree $0$ rational components away from the genus $1$ singularity and on the degree $0$ branches of the $(m-k+h)$-fold which are not contracted by $\rho.$ 
 
Since $\rho^*\hat{f}^*\OO_{\PP^r}(1)(\Sigma)$ has no $H^1$ it satisfies cohomology and base change. Then we can check  that the result of this second contraction is a $(k,m-k)$-singularity just looking at the vanishing orders of the sections of $\rho^*\hat{f}^*\OO_{\PP^r}(1)(\Sigma)$ on the central fiber $\bar{C}$ of the first contraction. These sections are constant on the degree 0 branches we did not mark and so  we conclude that the dependence relation among  the tangent vectors of the branches at the singular point of the second contraction only involve the $m-k$ rational tail image of the ones adjacent to $E$.
...
Thus it is enough to verify that $H^1(\hat{C},\hat{L})=0$ . Once done this we are going to obtain the smoothing of the original map by precomposing with the contraction $F\colon\cC\to\hat{\cC}\xrightarrow{\hat F}\PP^r$.
...
We introduce the moduli space $\Mone{1}{\PP^4}{d}^p$ of $1$-stable curves with $p$-fields, we endow it with a $0$-dimensional virtual class and show that its degree coincides with the genus $1$ cuspidal invariants of the quintic three-fold $X$ up to a sign. This is a word-by-word repetition of the arguments in \cite{CLpfields} once noticed that they carry over to the situation where we work over a family of Gorenstein (not necessarily nodal) curves. Our aim in this section is to improve the legibility of our paper by providing the non-expert reader with a r\'{e}sum\'{e} of some of the key ideas contained in \cite{CLpfields}; it could otherwise be skipped.
\subsection{Moduli of sections}
\begin{dfn}
 Let $B$ be an algebraic stack and let $\pi\colon \cC\to B$ be a flat proper morphism of finite presentation which is representable by algebraic spaces and whose geometric fibers are reduced l.c.i. curves. Let $\mathcal Z$ be an  Artin stack smooth over  $\cC$ such that the morphism to $\cC$ is representable and quasi- projective. For any $ S\to B$  the \emph{cone of section} of $\mathcal Z$ over $S$ is defined by:
 \[\mathfrak{S}(S)=\left\{\cC_S\;\text{morphisms}\; s\colon \cC_S\to\mathcal Z_S\right\};\]
 the arrow in $\mathfrak{S}(S)$ are defined in the obvious way.
 The groupoid $\mathfrak{S}$ is an algebraic stack \cite[Proposition~2.3]{CL}.
 
\end{dfn}
 To fix notation, let us write down the following diagram
 \bcd
& \mathcal Z\ar[d] \\
\cC_{\mathfrak S}\ar[r]\ar[ur,bend left,"\mathfrak e"]\ar[d,"\pi_{\mathfrak S}"] & \cC\ar[d,"\pi"] \\
\mathfrak S \ar[r] & B
\ecd
where $\mathfrak e$ denotes the tautological evaluation map. 
...
We  abbreviate $\pazocal P:=\Mone{1}{\PP^4}{d}^p$. 
This should be thought of as the space of sections of the line bundle $\mathcal P_{\rm M}$ over $\hat\cC_{\rm M}$:

Otherwise, in parallel with the above description on the moduli space of 1-stable maps,  $\pazocal P$ can be thought as a (open in a) moduli space of sections of the bundle $\rm{Vb}(\hat{\mathcal L}_{\hP}^{\oplus 5}\oplus \mathcal P_{\hP}). $
%\bcd
...
In the notation of the previous section,
 the morphism $\mathfrak S\to B$ admits a relative dual perfect obstruction theory:
...
The proof of this fact is given in \cite[Proposition 2.5]{CLpfields}, and it is enough to notice that it relies on general properties of obstruction theories and the cotangent complex and the fact that $\mathcal Z\to\cC$ is smooth,
but never on the specification that $\cC\to B$ be a family of \emph{nodal} curves, to conclude that the same results apply when the fibers of $\cC\to B$ are l.c.i curves.

In the examples considered above we get:
 in the case of a line bundle we recover $\mathbb E_{C(\pi_*\mathcal L)/B}=\R^\bullet\pi_*\mathcal L$;
%The construction for a general smooth $\pazocal Z$ is way more flexible. 

in the case of the moduli space $\rm{M}=\oM_1^{(1)}(\PP^4,d)$ we get an obstruction theory relative to  $\hP$ given by $ \R^\bullet\hat\pi_{\rm M,*}(\bigoplus_0^4 \mathcal L_{\rm M})$;

in the case of 1- stable maps with fields $\pazocal P$ we get $\R^\bullet\hat\pi_{\pazocal P,*}(\mathcal L_{\pazocal P}^{\oplus 5}\oplus\mathcal P_{\pazocal P}).$

On the moduli space of 1-stable map we have another natural perfect obstruction theory relative to $\hM,$ which is the standard one defined by Behrend-Fantechi \cite[Proposition 6.3]{BF}. The next result explain how to compare the two.
\begin{lem}
...
\begin{proof}
 ...
 Finally, the fact that  $\mathbb T_{\rm M/\hP}\to \mathbb E_{\rm M/\hP}:=\R^\bullet\hat\pi_{\rm M,*}(\bigoplus_0^4 \mathcal L_{\rm M})$ gives the standard virtual class for the moduli space of maps follows from functoriality of virtual pullback \cite{Manolache-pullback}.
\end{proof}
Finally notice that the relative perfect obstruction theory for $\pazocal P$ fits in the following exact triangle:
\begin{lem}
 ...
 \ecd
\end{lem}
This follows from the general theory of moduli spaces of sections recalled above.

Notice that the virtual rank of $\mathbb E_{\pazocal P/\hP}:=\R^\bullet\hat\pi_{\pazocal P,*}(\mathcal L_{\pazocal P}^{\oplus 5}\oplus\mathcal P_{\pazocal P})$ is $0$, hence it endows the moduli space of $1$-stable maps with $p$-fields with a cycle class of dimension $0.$ In the next section we show that this cycles is actually supported on the proper sub-stack $\Mone{1}{X}{d}$ where 
 $X=V(\w)\subseteq\PP^4$ is the smooth quintic three-fold defined by a homogeneous polynomial
 $\w\in k[x_0,\ldots,x_4]_5.$ 
...
\subsection{Cosection Localization}
We show the existence of a cosection of the obstruction bundle whose degeneracy locus is the proper substack $\Mone{1}{X}{d}.$ The machinery of cosection localised virtual classes \cite{KLcosection}. Recall their Theorem 1.1:
...
The degeneracy locus of this cosection is $\Mone{1}{X}{d}$: by Serre duality if $\w(u)\neq 0$ then we can find a $p$ such that the cosection does not vanish; similarly we can do if $\psi\sum_{i=0}^4\partial_i\w(u)\neq 0$, but then $\psi=0$ by smoothness of $\w$. 

Moreover, Chang-Li prove that
$\sigma_1$ lifts to a cosection of the absolute obstruction bundle $\rm{Ob}_{\pazocal P}\to\OO_{\pazocal P}$ with the same degeneracy locus.\marginpar{Why is this needed}

We may thus endow $\Mone{1}{\PP^4}{d}^p$ with a localised virtual cycle:
\[[\pazocal P]^{\rm{vir}}_{\sigma_1}=0^{!}_{\sigma_1,\rm{loc}}[\mathfrak{C}_{\pazocal P/\hP}]\;\in\; A_0(\Mone{1}{X}{d}).\]


We we want to show that it gives the same numerical invariants as the cuspidal Gromov-Witten theory of $X$, up to a sign:
\begin{thm}
...
Similarly $\pazocal V:=\Mone{1}{V}{(d,0)}^p$ can be defined as the cone of sections of $\mathcal Z:=\mathcal Z'\oplus\rm{Vb}(\mathcal P_{\hP})$. The general theory explained above provides an obstruction theory for $\pazocal V\to\hP\times\Aaff^1_t$ \cite[Proposition 4.2]{CLpfields}:
...
This issue is solved in \cite[\S\S4.5-6]{CLpfields} by lifting the obstruction theory to 
\[\phi_{\pazocal V/\hP}\colon\mathbb T_{\pazocal V/\hP}\to\mathbb E_{\pazocal V/\hP},\]
 where the latter fits into an exact triangle:
\[ \R^1\hat\pi_{\pazocal V,*}\OO_{\hat\cC_{\pazocal V}}[-2]\to\mathbb E_{\pazocal V/\hP}\xrightarrow{\nu}\mathbb E_{\pazocal V/\hP}\xrightarrow{[1]}.\]
Furthermore the lifted obstruction theory is compatible with the one the fibers, 
\begin{lem}
There is the following commutative diagram of exact triangles:
\bcd
\hat{\pi}_{\pazocal V_c *}\OO_{\hat\cC_{\pazocal V_c}}[-1]\ar[r] & \mathbb E_{\pazocal V_c/\hP} \ar[r] &\mathbb E_{\pazocal V/\hP}\rvert_{\pazocal V_c}\\
\mathbb{T}^{\geq 1}_{\pazocal V_c/\pazocal V}\ar[u]\ar[r] & \mathbb{T}^{\geq 1}_{\pazocal V_c/\hP}\ar[u]\ar[r] & \mathbb{T}^{\geq 1}_{\pazocal V/\hP}\rvert_{\pazocal V_c} \ar[u]
\ecd
\end{lem}
The proof is exactly the same given in \cite[\S~4.6]{CL}.
...
%They notice that the fibers of $v$ are vector spaces and the obstruction theory $\mathbb E_{\pazocal N/\pazocal Q}$ is symmetric, so it can be represented as 
%\[[T_{v^{-1}(\xi)}\xrightarrow{0}T^*_{v^{-1}(\xi)}]\] along the fibers. This is enough since we are only interested in the action of $v^!_{\mathbb E_{\pazocal N/\pazocal Q},\rm{loc}}$ on $A_0(\pazocal Q)$ \cite[Theorem 5.7]{CLpfields}.
