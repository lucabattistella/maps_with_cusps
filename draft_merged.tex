...
\subsection{Cosection-localised virtual pullback}
In this section we slightly generalise the notion of virtual pullback \cite{Manolache-pullback} to localised cycles and we show that this operation commutes with proper pushforward. 
We start by recalling the construction of \cite{KLcosection} for the benefit of the non-expert reader.

Let $Y\to S$ be a morphism of algebraic stacks of DM type and let $E$ be a vector bundle over $Y$ admitting a cosection
...
\[G=\operatorname{Ker}\left(E\rvert_U\to\OO_Y\rvert_U\right),\qquad E(\sigma)=E\rvert_{Y(\sigma)}\cup G.\]
 Kiem and Li define a localized Gysin map:
 ...
 Let $[B]\in Z_*(E(\sigma))$ be a cycle represented by a closed integral substack. If $B\subset  E\rvert_{Y(\sigma)}$ then
  they just consider the standard Gysin morphism: \[s^!_{\sigma,\rm{loc}}[B]:=s^!_{E\rvert_{Y(\sigma)}}[B]\in A_*(Y(\sigma)).\]
 Suppose now that $B$ is not contained in $E\rvert_{Y(\sigma)}.$ Then we may choose a variety $X\xrightarrow{\nu} Y$ such that:
 ...
 for a Cartier divisor $D\subseteq X.$ The restriction of $\nu$ to the divisor gives a proper morphism $\nu(\sigma)\colon D\to Y(\sigma);$
...
\[\OO b_{Y/S}\to \OO_Y\]
that lifts to a cosection of the absolute obstruction sheaf $\OO b_Y$.
Then we can define a localised virtual pullback operation: \[(\rho')^!_{\mathbb E_{Y'/S'},\sigma'}\colon A_*(S')\to A_{*-\operatorname{rk}(\mathbb E)}(Y'(\sigma'))\] where $\sigma'=(\varphi')^*(\sigma)$ and $Y'(\sigma')=Y(\sigma)\times_{Y} Y'$.
\end{defprop}
Replace $S'$ by a primitive cycle in it, i.e. assume that $S'$ is irreducible and reduced. Recall that pulling back $\mathbb E_{Y/S}$ along $\varphi'$ endows $\rho'$ with a relative perfect obstruction theory $\mathbb E_{Y'/S'}:=(\varphi')^*\mathbb E_{Y/S}.$ Furthermore notice that we can pullback the cosection:
\[\OO b_{Y'/S'}\cong \varphi^* \OO b_{Y/S}\xrightarrow{\sigma'}\OO_{Y'}.\]
Notice that since $\sigma$ lifts to the absolute obstruction sheaf, $\sigma'$ does as well. The degeneracy locus of $\sigma'$ is $Y'(\sigma')=Y(\sigma)\times_{Y} Y',$ which in turns implies that:
...
By \cite[Corollary 4.5]{KLcosection} and by the cartesian diagram above,
...
Indeed consider on $\Z^p$ the perfect obstruction theory: \[\mathbb E_{\Z^p/\XP}:=\alpha^*\left(\mathbb E_{\Mone{1}{\PP^4}{d}^p/\mathfrak{Pic}_1{(1)}}\right)\] and the induced cosection
$\alpha^*\sigma\colon \OO b_{\Z^p/\XP}\to \Z^p$
with degeneracy locus:
\[\pazocal D(\alpha^*\sigma)=\Mone{1}{X}{d}\times_{\Mone{1}{\PP^4}{d}^p} \Z^p.\]
Then we have that
\begin{align*}
\mathfrak C_{\Z^p/\XP}& \subseteq \alpha^*\mathfrak C_{\Mone{1}{\PP^4}{d}^p/\mathfrak{Pic_1}{(1)}}\\
& \subseteq \alpha^* h^1/h^0\left(\mathbb E_{\Mone{1}{\PP^4}{d}^p/\mathfrak{Pic}_1{(1)}}\right)(\sigma)\\
&\cong h^1/h^0\left(\mathbb E_{\Z^p/\XP}\right)(\alpha^*\sigma)
\end{align*}
and we can thus define $[\Z^p]^{\rm{vir}}_{\alpha^*\sigma,\rm{loc}}:=\gamma^!_{\mathbb E_{\Z^p/\XP},\alpha^*\sigma}[\XP].$
\begin{lem}\label{lem:comutativity}
With the same setup and notation as in the definition of localised virtual pullback, suppose furthermore that $\varphi$ is proper and $\varphi'(Y')\cap U\neq\emptyset$. For every cycle $[B']\in A_*(S')$,
\[\phi'_*\circ(\rho')^!_{\mathbb E_{Y'/S'}}[B']=(\rho)^!_{\mathbb E_{Y/S}}\circ\phi_*[B]\in A_*(Y(\sigma)).\]
...
\ecd
Only the left-most square is not classical. Hence we are left with showing that for every cycle $[B']\in A_*(h^1/h^0(\mathbb{E}_{Y'/S'})(\sigma'))$ we have:
...
Assume then that $B'$ is a prime cycle in $h^1/h^0(\mathbb{E}_{Y'/S'})(\sigma')$ not contained in $h^1/h^0(\mathbb{E}_{Y'/S'})\rvert_{Y'(\sigma')}$. Then, according to the construction recalled at the beginning of this section, 
 \[s^!_{\sigma',\rm{loc}}[B']:=\frac{1}{k} \nu'(\sigma')_*\left([D]\cdot s^!_{\widetilde{G}}[\widetilde{B}]\right)\,\in\,A_*(Y'(\sigma')),\]
 where $\nu'\colon X\to Y'$, $(\nu')^*(\sigma')$ and $\widetilde B$ satisfy the conditions~\eqref{cond:deflocal}. Notice that if we consider the composition
 $\nu\colon X\xrightarrow{\nu'} Y'\xrightarrow{\varphi'} Y$ and the pull-back $\nu^*(\sigma)$, the first two conditions in \eqref{cond:deflocal} are satisfied. How about
 \[\widetilde{B}\to B'\to \varphi^{\mathbb{E}}(B')=:B?\]
 Suppose first that $B'\to B$ is generically finite of degree $h$; then $\widetilde{B}\to B$ satisfies the last condition in \eqref{cond:deflocal} and has degree $hk$, so:
 \begin{align*}
 \varphi'_*( s^!_{\sigma',\rm{loc}}[B'])&= \frac{1}{k}\varphi'_*\ \nu'(\sigma')_*\left([D]\cdot s^!_{\widetilde{G}}[\widetilde{B}]\right)\\
 &=\frac{1}{k} \nu(\sigma)_*\left([D]\cdot s^!_{\widetilde{G}}[\widetilde{B}]\right)\\
 &=h\, s_{\sigma,\rm{loc}}^![B]\\
...
\ecd
We may in fact assume that $\phi_{\eta}$ is an isomorphism, and then it is enough to take the semistable model of $\hC.$
\end{proof}
%We show that
